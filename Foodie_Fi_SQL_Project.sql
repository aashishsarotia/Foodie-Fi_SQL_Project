...
SELECT 
  ...
  ROUND((COUNT(DISTINCT customer_id) * 100.0 / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions)), 1) AS churned_customers_percent
FROM subscriptions
WHERE plan_id = 4;
...
SELECT 
  ...
  ROUND((COUNT(customer_id) * 100.0) / (SELECT COUNT(DISTINCT customer_id) FROM CTE), 1) AS customer_percent
FROM CTE
WHERE rn = 2
GROUP BY plan_name;
...
SELECT 
  P.plan_name,
  COUNT(CTE.customer_id) AS customer_count,
  ROUND((COUNT(CTE.customer_id) * 100.0) / (SELECT COUNT(DISTINCT customer_id) FROM CTE), 1) AS percent_of_customers
FROM CTE
...
-- Any monthly customer going to annual plan
...
SELECT 
  ROUND(AVG(annual_start - trial_start), 0) AS average_days_from_trial_to_annual
FROM TRIAL AS T
